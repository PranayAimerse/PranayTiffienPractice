  ...
  UserQueryResponse.init(
    {
      ...
      status: {
        type: DataTypes.ENUM('active', 'disable', 'blocked', 'deleted'),
      },
      updated_by: {
        type: DataTypes.STRING,
        defaultValue: null,
    ...
    {
      sequelize,
      modelName: 'UserQueryResponse',
      tableName: 'user_query_responses',
      timestamps: false,
      // hooks: {
      //   afterCreate: async (response) => {
      //     response.token = CreateToken('userQueryResponse');
      //     response.slug = CreateSlug(response.title || '');
      //     await response.save();
      //   },
      // },
    }
